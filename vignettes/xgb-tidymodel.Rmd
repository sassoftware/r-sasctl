...
``` r
path <- file.path(tempdir(), "xgbModel");

modelrda <- "Rxgb.rda"

dir.create(path, recursive = TRUE)

saveRDS(xgb_fitted, file.path(path, modelrda))
```
...
code <- codegen(xgb_fitted,
                path = file.path(path, "scoreCode.R"),
...
#> library("xgboost")
#> 
#> sasctlRmodel <- readRDS(file = paste(rdsPath, "Rxgb.rda"))
#> 
...
zip(file.path(path, "Rmodel.zip"), files = files_to_zip)
```
...
mod <- register_model(
  session = sess,
  file = file.path(path, "Rmodel.zip"),
  name = "Rxgb",
  type = "zip",
  project = "R_sasctl",
  force = TRUE
